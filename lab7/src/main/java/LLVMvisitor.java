...
public class LLVMvisitor extends SysYParserBaseVisitor<LLVMValueRef>{
    ...
    private ArrayList<LLVMBasicBlockRef> whileBlock = new ArrayList<>();
        ...
        for (SysYParser.VarDefContext varDefContext : ctx.varDef()) {
            ...
            if (varDefContext.ASSIGN() != null) {
                ...
                if (expContext != null) {
                    ...
                    LLVMValueRef arrLen = visit(varDefContext.constExp(0));
...
//            else return f0;
    ...
    public LLVMValueRef visitWhileStmt(SysYParser.WhileStmtContext ctx) {
        LLVMBasicBlockRef whileCondition = LLVMAppendBasicBlock(func, /*blockName:String*/"whileCondition");
        LLVMBasicBlockRef whileBody = LLVMAppendBasicBlock(func, /*blockName:String*/"whileBody");
        LLVMBasicBlockRef entry = LLVMAppendBasicBlock(func, /*blockName:String*/"entry");

        LLVMBuildBr(builder,whileCondition);
        LLVMPositionBuilderAtEnd(builder, whileCondition);//后续生成的指令将追加在 的后面

        LLVMValueRef condRe = visit(ctx.cond());
        if (LLVMTypeOf(condRe).equals(i32Type))
            condRe = LLVMBuildICmp(builder,LLVMIntNE,condRe,zero,"condRe");
        LLVMBuildCondBr(builder,condRe,whileBody,entry);


        whileBlock.add(whileBody);
        whileBlock.add(entry);
        whileFlag++;
        LLVMPositionBuilderAtEnd(builder, whileBody);//后续生成的指令将追加在 的后面
        visit(ctx.stmt());
        LLVMBuildBr(builder,whileCondition);

//        LLVMPositionBuilderAtEnd(builder, whileBody);//后续生成的指令将追加在 的后面
//        condRe = visit(ctx.cond());
//        if (LLVMTypeOf(condRe).equals(i32Type))
//            condRe = LLVMBuildICmp(builder,LLVMIntNE,condRe,zero,"condRe");

        LLVMBuildCondBr(builder,condRe,whileBody,entry);

        LLVMPositionBuilderAtEnd(builder, entry);//后续生成的指令将追加在 的后面
    ...
    public LLVMValueRef visitBreakStmt(SysYParser.BreakStmtContext ctx) {
        LLVMBuildBr(builder,whileBlock.get(whileFlag*2-1));
        return null;
    }

    @Override
    public LLVMValueRef visitContinueStmt(SysYParser.ContinueStmtContext ctx) {
        LLVMBuildBr(builder,whileBlock.get(whileFlag*2-2));
        return null;
    }
}
